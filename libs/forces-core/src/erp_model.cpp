...
ErpResult ErpAccelerationModel::evaluate(const astroforces::atmo::StateVector& state,
  ...
  if (sc.mass_kg <= 0.0 || config_.earth_reference_radius_m <= 0.0 || config_.earth_radiation_pressure_ref_pa < 0.0) {
    return ErpResult{.status = astroforces::atmo::Status::InvalidInput};
  }
  if (state.frame != astroforces::atmo::Frame::ECI && state.frame != astroforces::atmo::Frame::ECEF) {
    return ErpResult{.status = astroforces::atmo::Status::InvalidInput};
  }
  ...
  if (!(earth_dist_m > 0.0)) {
    return ErpResult{.status = astroforces::atmo::Status::NumericalError};
  }
